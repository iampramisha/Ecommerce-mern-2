...
const addToCart = async (req, res) => {
  try {
    ...
    // Find the product to get its total stock and branches
    ...
    const populatedCart = await Cart.findOne({ user: userId }).populate({
      ...
      select: 'image title price salePrice branches', // Include 'branches' here
    });
    ...
    const formattedItems = populatedCart.products.map(item => ({
      ...
      weight: item.weight,
      branches: item.product.branches || [], // Include branches here
    }));
